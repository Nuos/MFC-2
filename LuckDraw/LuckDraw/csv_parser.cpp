...
void csv_parser::ParseAllData(std::vector<SDataInfo> & inDataList)
{
	int count = total_lines();
	csv_file.clear(); //Clear State
	csv_file.seekg(0, std::ios::beg);
	std::string line;
	inDataList.empty();
	inDataList.resize(count - 1);
	for (int i = 1; i < count; i++)
	{
		getline(csv_file, line);
		size_t tStep = line.rfind(',');
		inDataList[i - 1].name = line.substr(0, tStep);
		inDataList[i - 1].phone = line.substr(tStep + 1, 11);
	}
	if (csv_file.bad())
		perror("Error while Reading");
}
